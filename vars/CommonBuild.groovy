...
def css = "<style>body{font-size: 3em}img{width: 50%;height:auto}.good-text{color:#7FD400}.bad-text{color:#F15D22}.good{background-color:#7FD400}.bad{background-color:#F15D22}.banner{width: 100%; min-height: 20px;padding: 20px}</style>";
    ...
    node{
        ...
        try{
            stage("Success Notification"){
                ...
                body: "<html><body>${css}<div class='good banner'>${embeddedImage}</div><a href='${env.BUILD_URL}'>${JOB_NAME} ${currentBuild.displayName} / ${currentBuild.result}</a><br/> ${duration} <br/> Build Type: ${buildTypeAction}</body></html>",
				mimeType: 'text/html'
            }
        }
        catch(err){
