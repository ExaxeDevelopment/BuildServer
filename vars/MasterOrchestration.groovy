		...
		stage("Orchestration - ${OrchestrationType}"){
			...
			switch (orchestrationType) {
				case "Development":
					...
					description = "MasterBuild-Orchestration-Development"
					break
				...
				case "Integration":
					...
					description = "MasterBuild-Orchestration-Integration"
					break
				...
				case "SuperBuild":
					...
					description = "MasterBuild-Orchestration-SuperBuild"
					break
			...
			if(orchestrationType == "PingAll" && mapStatuses.containsValue(false)){
				error("One or more jobs have failed. Please check the logs for details.")
			}

			List failedJobs = getPingFailedJobs(mapStatuses);
		...
		try{
			stage("${job}"){
				...
				def handle = triggerRemoteJob(remoteJenkinsName: serverName,
					job: job, 
					auth: TokenAuth(
						apiToken: Secret.fromString(token), 
						userName: "admin"
					), 					
					////token: token, 
					pollInterval: 30, 
...
def getDevelopmentSteps(buildFromStage, buildToStage){
	...
	map06.put("Deploy-Dev002-Hansard-Exaxe-Automation", APEX_BUILD_SERVER)
...
def getPingAllSteps(buildFromStage, buildToStage){
	Map<String,Map<String,String>> steps = new LinkedHashMap<String,Map<String,String>>();
	Map<String,String> map01 = new HashMap<String,String>();
	map01.put("Ping002", DISTRIBUTION_PLUS_BUILD_SERVER)
	map01.put("Ping003", ADVICE_PLUS_BUILD_SERVER)
	map01.put("Ping004", ADMIN_PLUS_BUILD_SERVER)
	map01.put("Ping005", APEX_BUILD_SERVER)
	map01.put("Ping006", API_BUILD_SERVER)

	map01.put("Ping007", APEX_BUILD_I_SERVER)
	map01.put("Ping008", API_BUILD_I_SERVER)

	map01.put("Ping009", ACN_DEPLOY_SERVER)
...
def getPingFailedJobs(mapStatuses){	
}
...
def getHansardDeployments(buildFromStage, buildToStage){
	...
	map02.put("Deploy-Dev002-Hansard-Exaxe-Automation", APEX_BUILD_SERVER)
...
def getSuperBuildSteps(buildFromStage, buildToStage){
	...
	map10.put("Deploy-Dev002-Hansard-Exaxe-Automation", APEX_BUILD_SERVER)
